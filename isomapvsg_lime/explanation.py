...
from vanilla_lime.exceptions import LimeError
...
class Explanation(object):
    ...
    def __init__(self,
        ...
        # self.score = {}                       # 分数
        self.mae = None                         # mae分数
        self.local_pred = None                  # 局部预测
        self.used_features = []
        if mode == 'classification':
            self.class_names = class_names     # 类别名称
            self.top_labels = None             # 要解释的top标签，后面编程一个索引列表
            self.predict_proba = None          # 类别概率
        elif mode == 'regression':
            self.class_names = ['negative', 'positive']
            self.predicted_value = None       # 预测值
            self.min_value = 0.0              # 最小值
            self.max_value = 1.0              # 最大值
            self.dummy_label = 1              # 虚拟标签
        else:
    ...
    def as_list(self, label=1, **kwargs):
        ...
        # print('explanation142行的ans值为：', ans)
        ans = [(x[0], float(x[1])) for x in ans]
        # print('explanation144行的ans值为：', ans)
        return ans
    ...
    def as_pyplot_figure(self, label=1, **kwargs):
        ...
        plt.barh(pos, vals, align='center', color=colors)   # 柱状图
